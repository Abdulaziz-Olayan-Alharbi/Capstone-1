...
public class MerchantStockController {
    ...
    @PutMapping("/additional/{productId}/{merchantId}/{stock}")
    public ResponseEntity addStock (@PathVariable int productId , @PathVariable int merchantId , @PathVariable int stock){
        if (merchantStockService.addStock(productId,merchantId,stock).equals("true")) {
            return ResponseEntity.status(200).body(new ApiResponse("Stock Added Successfully"));
        }
        return ResponseEntity.status(400).body(merchantStockService.addStock(productId,merchantId,stock));
    }
